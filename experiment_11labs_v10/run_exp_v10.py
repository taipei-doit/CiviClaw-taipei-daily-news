...
WORK_DIR = Path.home() / "tw-gov-video" / "experiment_11labs_v10"
...
def main():
    content = {
        "en": {
            "intro": {
                "title": "Taipei City News", 
                "tts": "Taipei City News Update, created by AI."
            },
            "content": {
                "title": "Taipei & Himeji Sign Historic Friendship Pact", 
                "script": "Taipei Mayor Chiang Wan-an and Himeji Mayor Hideyasu Kiyomoto signed a friendship exchange agreement today. The pact focuses on youth talent cultivation, tourism marketing, and economic cooperation.", 
                "tts": "Taipei Mayor Chiang Wan-an and Himeji Mayor Hideyasu Kiyomoto signed a friendship exchange agreement today. The pact focuses on youth talent cultivation, tourism marketing, and economic cooperation."
            },
            "outro": {
                "title": "Thank You", 
                "script": "Please subscribe and share!", 
                "tts": "Thank you for watching Taipei City News."
            }
        },
        "ja": {
            "intro": {
            ...
            "content": {
                "title": "台北市と姫路市が歴史的な友好交流協定を締結", 
                "script": "蒋万安台北市長と清元秀泰姫路市長は本日、友好交流協定に署名しました。この協定は、青少年の人材育成、観光マーケティング、経済協力を中心としています。", 
                "tts": "蒋万安台北市長と清元秀泰姫路市長は本日、友好交流協定に署名しました。この協定は、青少年の人材育成、観光マーケティング、経済協力を中心としています。"
            },
            "outro": {
                "title": "おわり", 
    ...
    with sync_playwright() as p:
        ...
        for lang, sections in content.items():
            ...
            for slide_type in ["intro", "content", "outro"]:
                ...
                audio_path = generate_tts(data["tts"], f"v10_audio_{slide_type}_{lang}.mp3")
                
                html_path = create_single_slide_html(slide_type, lang, data)
                page.goto(f"file://{html_path}")
                page.wait_for_timeout(500)
                
                img_path = WORK_DIR / f"v10_frame_{slide_type}_{lang}.png"
                page.screenshot(path=str(img_path))
                
                clip_path = WORK_DIR / f"v10_clip_{slide_type}_{lang}.mp4"
                make_clip(img_path, audio_path, clip_path)
                clips.append(f"file '{clip_path.name}'")
                
            concat_file = WORK_DIR / f"v10_concat_{lang}.txt"
            concat_file.write_text("\n".join(clips))
            
            final_video = WORK_DIR / f"final_v10_{lang}.mp4"
