...
1. **12:00 PM (12:00) Fetch:**
   - If current Taipei time is past 12:00 PM:
     ...
     - If it is NOT today's Taipei date, you must run the fetch:
       - `curl -s "https://www.gov.taipei/OpenData.aspx?SN=ABBF62618F53F8DE" > output/news_12pm.json`
...
2. **5:00 PM (17:00) Pipeline:**
   - If current Taipei time is past 17:00 (5:00 PM):
     ...
     - **CRITICAL LOCK & CRASH RECOVERY:** 
       - If `5pm_pipeline_date` is exactly today's Taipei date (e.g. "2026-04-26"), you MUST STOP AND DO NOTHING.
       - If `5pm_pipeline_date` is "running": Check the last modified time of `memory/heartbeat-state.json`. If it was modified more than 4 hours ago, or on a previous day, the pipeline is considered crashed. You should reset it and proceed with the run. Otherwise, if it was modified recently, STOP and reply `HEARTBEAT_OK`.
     - If `5pm_pipeline_date` is NOT today's Taipei date, then you must run the 5 PM pipeline.
     - **HOW TO RUN THE PIPELINE:**
       - **CRITICAL CWD:** You must run all commands from the repository root (workspace root) directory.
       ...
       - Step 3: YOU MUST USE YOUR LLM BRAIN TO EVALUATE THE ARTICLES AND WRITE THE JSON YOURSELF using the write tool. DO NOT RUN ANY PYTHON SCRIPT TO DO THE SELECTION. Select the 5 best articles and manually write them to `output/selected_articles.json`. 
       ...
       - Step 4: Delete old audio: `rm -f output/voice_*.mp3`
       - Step 5: Run the exact command: `python3 scripts/inject_images.py && python3 scripts/generate_images.py && python3 scripts/tts.py && python3 scripts/render_html.py && python3 scripts/render_video.py && python3 scripts/upload_youtube.py && python3 scripts/deploy_web.py && python3 scripts/deploy_podcast.py && git add docs/ && (git diff --staged --quiet || git commit -m "Auto-update website and podcast") && git push origin main && python3 scripts/send_line.py`
