...
def main():
    ...
    with sync_playwright() as p:
        ...
        for lang, sections in content.items():
            ...
            page.wait_for_timeout(1000) # Ensure fonts load
            ...
            for slide_type in ["intro", "content", "outro"]:
                # Generate TTS
                audio_path = generate_tts(sections[slide_type]["tts"], f"audio_{slide_type}_{lang}.mp3")
            ...
            # Add final frame again to satisfy ffmpeg concat behavior
            concat_lines.append(f"file 'frame_outro_{lang}.png'")
            concat_lines.append(f"duration 0.5")
            
            concat_file = WORK_DIR / f"concat_{lang}.txt"
            concat_file.write_text("\n".join(concat_lines))
            
            # Combine
            temp_video = WORK_DIR / f"temp_video_{lang}.mp4"
            temp_audio = WORK_DIR / f"temp_audio_{lang}.mp3"
            final_video = WORK_DIR / f"final_v3_{lang}.mp4"
            
            # Render video from frames
            subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(concat_file), "-fps_mode", "vfr", "-pix_fmt", "yuv420p", str(temp_video)], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            
            # Concat audio
            filter_complex = "".join([f"[{i}:0]" for i in range(len(audio_inputs))]) + f"concat=n={len(audio_inputs)}:v=0:a=1[out]"
            audio_cmd = ["ffmpeg", "-y"]
            for a in audio_inputs:
                audio_cmd.extend(["-i", a])
            audio_cmd.extend(["-filter_complex", filter_complex, "-map", "[out]", str(temp_audio)])
            subprocess.run(audio_cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            
            # Merge
            subprocess.run(["ffmpeg", "-y", "-i", str(temp_video), "-i", str(temp_audio), "-c:v", "copy", "-c:a", "aac", "-shortest", str(final_video)], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
