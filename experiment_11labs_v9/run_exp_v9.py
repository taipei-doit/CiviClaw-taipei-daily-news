...
WORK_DIR = Path.home() / "tw-gov-video" / "experiment_11labs_v9"
    ...
    content = {
        "en": {
            ...
            "content": {
                ...
                "script": "Taipei Mayor Chiang Wan-an and Himeji Mayor Hideyasu Kiyomoto signed a memorandum of understanding today. The pact marks a milestone in bilateral relations, promoting cooperation in business, startup incubation, and cultural exchange.", 
                "tts": "Taipei Mayor Chiang Wan-an and Himeji Mayor Hideyasu Kiyomoto signed a memorandum of understanding today. The pact marks a milestone in bilateral relations, promoting cooperation in business, startup incubation, and cultural exchange."
            },
        ...
        "ja": {
            ...
            "content": {
                ...
                "script": "蒋万安台北市長と清元秀泰姫路市長は本日、覚書に署名しました。この協定は二国間関係の節目となり、ビジネスやスタートアップ支援、文化交流における協力を促進します。", 
                "tts": "蒋万安台北市長と清元秀泰姫路市長は本日、覚書に署名しました。この協定は二国間関係の節目となり、ビジネスやスタートアップ支援、文化交流における協力を促進します。"
            },
    ...
    with sync_playwright() as p:
        ...
        for lang, sections in content.items():
            ...
            for slide_type in ["intro", "content", "outro"]:
                ...
                audio_path = generate_tts(data["tts"], f"v9_audio_{slide_type}_{lang}.mp3")
                ...
                img_path = WORK_DIR / f"v9_frame_{slide_type}_{lang}.png"
                page.screenshot(path=str(img_path))
                
                clip_path = WORK_DIR / f"v9_clip_{slide_type}_{lang}.mp4"
                make_clip(img_path, audio_path, clip_path)
                clips.append(f"file '{clip_path.name}'")
                
            concat_file = WORK_DIR / f"v9_concat_{lang}.txt"
            concat_file.write_text("\n".join(clips))
            
            final_video = WORK_DIR / f"final_v9_{lang}.mp4"
