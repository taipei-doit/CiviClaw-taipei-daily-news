...
WORK_DIR = Path.home() / "tw-gov-video" / "experiment_11labs_v5"
...
def generate_tts(text, filename, pad_seconds):
    ...
    try:
        ...
        # Pad the audio with silence so the slide stays on screen
        subprocess.run(f"ffmpeg -y -i {raw_path} -af 'apad=pad_dur={pad_seconds}' {out_path}", shell=True, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return out_path
...
<body>
    ...
    <div id="slide_intro" class="slide title-slide" style="display: flex;">
    ...
    with sync_playwright() as p:
        ...
        for lang, sections in content.items():
            ...
            for slide_type in ["intro", "content", "outro"]:
                audio_path = generate_tts(sections[slide_type]["tts"], f"v5_audio_{slide_type}_{lang}.mp3", pad_times[slide_type])
                
                # Switch slide via JS
                page.evaluate(f"showSlide('slide_{slide_type}')")
                
                # IMPORTANT: Wait 1 full second for DOM update and painting before screenshotting
                page.wait_for_timeout(1000)
                
                img_path = WORK_DIR / f"v5_frame_{slide_type}_{lang}.png"
                page.screenshot(path=str(img_path))
                
                clip_path = WORK_DIR / f"v5_clip_{slide_type}_{lang}.mp4"
                make_clip(img_path, audio_path, clip_path)
                clips.append(f"file '{clip_path.name}'")
                
            concat_file = WORK_DIR / f"v5_concat_{lang}.txt"
            concat_file.write_text("\n".join(clips))
            
            final_video = WORK_DIR / f"final_v5_{lang}.mp4"
