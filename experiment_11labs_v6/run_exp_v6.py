...
WORK_DIR = Path.home() / "tw-gov-video" / "experiment_11labs_v6"
...
def main():
    content = {
        "en": {
            "intro": {"title": "Taipei City News", "tts": "Taipei City News Update."},
            "content": {
                "title": "Taipei & Fukuoka MOU", 
    ...
    with sync_playwright() as p:
        ...
        for lang, sections in content.items():
            ...
            for slide_type in ["intro", "content", "outro"]:
                ...
                audio_path = generate_tts(data["tts"], f"v6_audio_{slide_type}_{lang}.mp3")
                ...
                img_path = WORK_DIR / f"v6_frame_{slide_type}_{lang}.png"
                page.screenshot(path=str(img_path))
                
                clip_path = WORK_DIR / f"v6_clip_{slide_type}_{lang}.mp4"
                make_clip(img_path, audio_path, clip_path)
                clips.append(f"file '{clip_path.name}'")
                
            concat_file = WORK_DIR / f"v6_concat_{lang}.txt"
            concat_file.write_text("\n".join(clips))
            
            final_video = WORK_DIR / f"final_v6_{lang}.mp4"
